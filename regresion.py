...
def regresion(datos, regr, tabControl2):
    ...
    if (model.intercept_ > 0):
        ...
        y_cad = "Y = " + str(round(model.coef_[0].item(), 3)) + "X - " + str(round(model.intercept_.item(), 3))
    tk.Label(regr, text="Ecuación Regresión Lineal: ").place(x=230, y=25)
    tk.Label(regr, text=y_cad).place(x=420, y=25)
    graphRegresion(x_train.reshape(-1, 1), y_train.reshape(-1, 1), x_reg.reshape(-1, 1), model, regr, tabControl2)


def graphRegresion(x_train, y_train, x_reg, model, regr, tabControl2):
    ...
    plt.plot([x_min, x_max], [y_min.item(), y_max.item()], color='black')
